...
def ingestrec(solrurl, content)

	uri = URI.parse(solrurl)
...
logpath = "Report/Logs/"
uploadpath = "Upload/"
solrurl = YAML.load_file(ARGV[0])[ARGV[1]]['url'][/\"(.*?)"/,1] + "/update/json/docs?commit=true"
...
for uploadfile in list_of_files
	...
	if prefix.length > 0
		...
		ingestedrecs = 0
		index = 1
		CSV.foreach(uploadpath+uploadfile, :headers => true) do |row|
			errmsg = validaterecord(row)

			if errmsg.length > 0
				errorcontent += "row" + index.to_s + ":" + errmsg + "\n\n"
			else
				solrdata = geojson(row)
				ingestrec(solrurl, solrdata)
				ingestedrecs += 1
			end
		...
		logfilename = logpath + prefix + Time.now().strftime("%Y%m%d%H%M%S") + ".log.txt"
		...
		if totalrecs != ingestedrecs
			errorfilename = errorpath + prefix + Time.now().strftime("%Y%m%d%H%M%S") + ".error.txt"
			createreport(errorfilename, uploadfile + "\n" + errorcontent)
		end

		# move upload file to archive folder
		FileUtils.mv(uploadpath + uploadfile, archivepath + uploadfile)
